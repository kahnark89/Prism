...
- **No visible score (Hard Line 6):** grounding is always rendered in the three plain-language bands, in narrative (Trajectory) or status-tile (Map) form — never a number, percentage, or progress bar. (A parent who wants the underlying continuous value for their own curiosity could find it one level down — an open question, §8.)
...
1. **Naming** — "the Map" / "the Trajectory" are working names for the two views; do they fit the device's voice, or does this need its own metaphor (an atlas? a garden? a constellation — ties nicely to the LED ring)?
2. **Numeric confidence, exposed or not** — should the parent suite offer *any* drill-down to the underlying continuous confidence value (for a data-curious parent), or does even that risk turning "owns it" into a number to optimize toward — for the *parent* this time, not the child? Leaning toward: keep it banded everywhere, in keeping with "optimize the teaching, never the measurement-as-metric" — but this is a judgment call worth your sign-off.
3. **Counter-balance discoverability** — how prominent should this control be? Too prominent risks framing parenting as a dial-turning exercise (anti-pattern for the "aliveness, not a system" feel); too hidden risks the one tool that makes Principle 12's promise *usable* going unused. Possibly: surfaced contextually (the suite notices a skew and *offers* the tool) rather than sitting as a permanent control — though "the suite notices a skew" is itself a design commitment worth scrutiny (it's analysis *of the parent's choices*, which needs its own care not to feel surveilled).
